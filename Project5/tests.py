...
class GraphTests(unittest.TestCase):
    ...
    def test_detect_cycle(self):
        ...
        for i, id in enumerate(string.ascii_lowercase):
            if i+1 < len(letters):
                graph.add_to_graph(id, letters[i+1], 1)
        graph.add_to_graph('a', 'z', 1)
        graph.add_to_graph('z', 'fin', 1)
        self.assertFalse(graph.detect_cycle())
    ...
    def test_a_star(self):
        ...
        #=== (B) Among Us graph tests ===#
        graph = Graph(csv='test_csvs/astar/among_us_graph_csv.csv')
        # now must set of coordinates for each vertex:
        positions = [(0,0), (2,0), (4,0), (7,0), (10,0), (12,0), (2,5), (6,4), (12,5), (5,9), (8,8), (12,8), (8,10), (0,2),
        ...
        for metric in [Vertex.euclidean_distance, Vertex.taxicab_distance]:
            ...
            # (3) test Reactor to Cafeteria shortest path in both directions
            # (shortest path is unique, so each heuristic will return the same path)
            subject = graph.a_star('Reactor', 'Cafeteria', metric)
            solution = (['Reactor', 'A', 'B', 'G', 'J', 'M', 'Cafeteria'], 22)
            self.assertEqual(subject, solution)
            graph.reset_vertices()

            subject = graph.a_star('Cafeteria', 'Reactor', metric)
            solution = (['Cafeteria', 'M', 'J', 'G', 'B', 'A', 'Reactor'], 22)
            self.assertEqual(subject, solution)
            graph.reset_vertices()

            # (4) test Reactor to Communications shortest path - bypass slow Electrical path
            # (shortest path is unique, so each heuristic will return the same path)
            subject = graph.a_star('Reactor', 'Communications', metric)
            solution = (['Reactor', 'A', 'B', 'G', 'H', 'D', 'E', 'Communications'], 35)
            self.assertEqual(subject, solution)
            graph.reset_vertices()

            subject = graph.a_star('Communications', 'Reactor', metric)
            solution = (['Communications', 'E', 'D', 'H', 'G', 'B', 'A', 'Reactor'], 35)
            self.assertEqual(subject, solution)
            graph.reset_vertices()

            # (5) test Communications to Navigation shortest path - bypass slow Shields path
            # (shortest path is unique, so each heuristic will return the same path)
            subject = graph.a_star('Communications', 'Navigation', metric)
            solution = (['Communications', 'E', 'D', 'H', 'G', 'J', 'K', 'L', 'Navigation'], 35)
            self.assertEqual(subject, solution)
            graph.reset_vertices()

            subject = graph.a_star('Navigation', 'Communications', metric)
            solution = (['Navigation', 'L', 'K', 'J', 'G', 'H', 'D', 'E', 'Communications'], 35)
            self.assertEqual(subject, solution)
            graph.reset_vertices()

            # (6) test Cafeteria to Med Bay - despite equal path lengths, A* heuristic will always prefer search to the left
            # (both heuristics will prefer the same path)
            subject = graph.a_star('Cafeteria', 'Med Bay', metric)
            solution = (['Cafeteria', 'M', 'J', 'K', 'Med Bay'], 8)
            self.assertEqual(subject, solution)
            graph.reset_vertices()

            subject = graph.a_star('Med Bay', 'Cafeteria', metric)
            solution = (['Med Bay', 'K', 'J', 'M', 'Cafeteria'], 8)
            self.assertEqual(subject, solution)
            graph.reset_vertices()
